...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    # we only need 1 pose (use most recent pose to keep dimensionality of omega small)
    ...
    for k in range(len(data)):
        ...
        temp = [*range(4, dim+2)]
        index_list = [0,1] + temp
        omega = omega.expand(dim+2,dim+2,index_list,index_list)
        xi = xi.expand(dim+2,1,index_list,[0])
