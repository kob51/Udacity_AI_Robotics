...
def main(grid, init, goal, steering_noise, distance_noise, measurement_noise, 
    ...
    for i in range(len(path.spath)):
        rs.append(5-path.spath[i][0]-0.5)
        cs.append(path.spath[i][1]+0.5)
    plt.scatter(cs,rs,color="red")
...
#plt.plot(plan.spath[:][0],plan.spath[:],[1])

print("Goal reached?:", result[0])
print("Number of collisions:",result[1])
print("Number of steps:",result[2])
