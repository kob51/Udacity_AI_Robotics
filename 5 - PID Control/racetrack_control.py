...
class robot:
    ...
    # --------
    # get: 
    #	retrieves current robot coordinates
    #
...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
   # cross-track error
    def cte(self, radius):
        ...
        # race course has straightaway lengths of 2*radius, and curves of radius
        
        # cte is negative inside the track, positive outside the track
        
        if self.x < radius:
            # curve on left half of racetrack
            # (distance to center of curvature) - (actual distance to center of curvature)
            cte = sqrt((self.x - radius)**2 + (self.y - radius)**2) - radius
        elif self.x > 3.0 * radius:
            # curve on right half of racetrack
            # (distance to center of curvature) - (actual distance to center of curvature)
            cte = sqrt((self.x -  3.0 * radius)**2 + (self.y - radius)**2) - radius
        elif self.y > radius:
            # straightaway on top half of racetrack
            # distance to straightaway line
...
def run(params, radius, printflag = False):
    ...
    for i in range(N*2):
        ...
        if printflag:
            print(myrobot)
        x,y,theta = myrobot.get()
        x_coords.append(x)
        y_coords.append(y)
        
    return err / float(N), x_coords, y_coords

radius = 25.0
params = [10.0, 15.0, 0]
err,x,y = run(params, radius, False)
print('\nFinal control parameters [kp, kd, ki]: ', params, '\n Error ->', err)
fig, ax1 = plt.subplots(1, 1, figsize=(8, 8))
ax1.plot(x, y, 'g', label='actual path')
plt.ylim((-1,max(x)))
plt.show()